...
public final class VkTurnProfile {
     ...
     *
     * <p>Профиль из федерации это доступ, а не личный конфиг. Тумблер, задетый
     * пальцем, ломает связь, и человек идёт не в настройки разбираться, а
     * писать, что у нас не работает. Плюс когда в инфраструктуре звонков что-то
     * меняется, чинится это сменой значения на сервере, а не выкаткой версии и
     * ожиданием, пока все обновятся.
     *
     * <p>Профиль от чужой панели сюда не попадает намеренно: там сервер держит
     * админ, и запирать его юзерам настройки от нашего имени не наше дело.
     */
    public boolean isSettingsManaged() {
        return wgProvisioned && FederationSubscription.ID.equals(subscriptionId);
    }
