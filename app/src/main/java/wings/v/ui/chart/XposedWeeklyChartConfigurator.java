...
public final class XposedWeeklyChartConfigurator {
    ...
    public static void bind(@NonNull BarChart chart, @NonNull List<XposedAttackStatsStore.DailyPoint> points) {
        ...
        chart.post(() -> {
            float leftOffset = isRtl
                ? DisplayDensityUtils.dpToPx(context, 30)
                : DisplayDensityUtils.dpToPx(context, 12);
            float rightOffset = isRtl
                ? DisplayDensityUtils.dpToPx(context, 10)
                : DisplayDensityUtils.dpToPx(context, 36);
            chart.setViewPortOffsets(
                leftOffset,
                DisplayDensityUtils.dpToPx(context, 8),
                rightOffset,
                DisplayDensityUtils.dpToPx(context, 34)
            );
            chart.notifyDataSetChanged();
